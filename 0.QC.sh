...
outDir=$resultsDir"/tables/QC"
mkdir -p $outDir

types=( "filter" "merged" "cutadapt" "dedup" )
for type in ${types[@]}; do

  QCdir=$resultsDir"/"$projectname"."$type
  rm $outDir"/$type.counts.txt"
  for file in `ls $QCdir`; do
    echo -n ".";
    echo -e -n "$file\t" | sed s/_L001.*\\t/\\t/ >>$outDir"/$type.counts.txt";
    wc -l $QCdir/$file | cut -f 1 -d " " >>$outDir"/$type.counts.txt";
  done;
...
QCdir=$inDir
rm $outDir"/raw.counts.txt"
for file in `ls $QCdir`; do
  echo -n ".";
  echo -e -n "$file\t" | sed s/_L001.*\\t/\\t/ >>$outDir"/$type.counts.txt";
  zless $QCdir/$file | wc -l >>$outDir"/$type.counts.txt";
done;
